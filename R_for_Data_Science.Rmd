...
```{r group_filter, eval=FALSE}
flights_sml %>%
          group_by(year, month, day) %>%
          filter(rank(desc(arr_delay)) < 10)
```
* Find all groups bigger than a threshold:
```{r group_filter2, eval=FALSE}
popular_dests <- flights %>%
        group_by(dest) %>%
        filter(n() > 365)
```
* Standardize to compute per group metrics:
```{r group_mutate, eval=FALSE}
popular_dests %>%
        filter(arr_delay > 0) %>%
...
```{r geom_freqpoly, echo=FALSE, fig.height = 2, fig.width=5, fig.align='center', cached = TRUE}
...
```{r covariation3, fig.height = 3, fig.width=4, fig.align='center', cached = TRUE}
ggplot(data = mpg) +
    geom_boxplot(
...
```{r geom_hex, fig.height = 3, fig.width=5, fig.align='center', cached = TRUE}
ggplot(data = smaller) +
...
```{r 2continuous1, fig.height = 2.5, fig.width=5, fig.align='center', cached = TRUE}
...
```{r 2continuous2, fig.height = 2.5, fig.width=5, fig.align='center', cached = TRUE}
ggplot(data = smaller, mapping = aes(x = carat, y = price)) +
    geom_boxplot(mapping = aes(group = cut_width(carat, 0.1)), varwidth = TRUE)
```
Another approach is to display approximately the same number of points in each bin. That’s the job of `cut_number()`:
```{r 2continuous3, fig.height = 2.5, fig.width=5, fig.align='center', cached = TRUE}
ggplot(data = smaller, mapping = aes(x = carat, y = price)) +
...
```{r case_tidy, eval=FALSE, fig.width = 10}
who
# A tibble: 7,240 x 60
...
```{r outer_join, out.width = "500px", fig.align="center", cached = TRUE, echo=FALSE}
knitr::include_graphics("outer_join.png")
```
...
```{r gss1, include=FALSE}
forcats::gss_cat
# [1] "year"    "marital" "age"     "race"    "rincome" "partyid" "relig"   "denom"   "tvhours"
...
```{r gss2, fig.align="center", fig.height=3, cached = TRUE}
...
```{r gss3, fig.align="center", fig.height=3, cached = TRUE}
rincome <- gss_cat %>%
      group_by(rincome) %>%
      summarize(age = mean(age, na.rm = TRUE), tvhours = mean(tvhours, na.rm = TRUE), n=n())
...
```{r chunk_options, out.width = "500px", fig.align="center", cached = TRUE, echo=FALSE}
knitr::include_graphics("chunk_options.png")
```
...
```{r label, fig.width = 6, fig.height= 4, fig.align="center", cached = TRUE}
...
```{r annotation1, fig.height = 4, fig.width=10, fig.align='center', cached = TRUE}
best_in_class <- mpg %>%
      group_by(class) %>%
...
```{r ggrepel, fig.height = 4, fig.align='center', cached = TRUE}
ggplot(mpg, aes(displ, hwy)) +
      geom_point(aes(color = class)) +
...
```{r theme, out.width = "500px", fig.align="center", cached = TRUE, echo=FALSE}
knitr::include_graphics("themes_default.png")
```
